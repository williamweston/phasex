...
init_params(void)
{
	...
	init_param_info(PARAM_STEREO_WIDTH,          "stereo_width",        "Stereo Width",PARAM_TYPE_REAL, -1, 127,  88,   0, 0,  8, 0, update_stereo_width,        NULL,               NULL);
	init_param_info(PARAM_AMP_VELOCITY,          "amp_velocity",        "Velocity",    PARAM_TYPE_INT,  -1, 127,   0,   0, 0,  8, 0, update_amp_velocity,        NULL,               NULL);
	init_param_info(PARAM_AMP_ATTACK,            "amp_attack",          "Attack",      PARAM_TYPE_INT,  -1, 127,   8,   0, 0,  8, 0, update_amp_attack,          NULL,               NULL);
	init_param_info(PARAM_AMP_DECAY,             "amp_decay",           "Decay",       PARAM_TYPE_INT,  -1, 127,  23,   0, 0,  8, 0, update_amp_decay,           NULL,               NULL);
	init_param_info(PARAM_AMP_SUSTAIN,           "amp_sustain",         "Sustain",     PARAM_TYPE_REAL, -1, 127, 108,   0, 0,  8, 0, update_amp_sustain,         NULL,               NULL);
	init_param_info(PARAM_AMP_RELEASE,           "amp_release",         "Release",     PARAM_TYPE_INT,  -1, 127,  37,   0, 0,  8, 0, update_amp_release,         NULL,               NULL);
	init_param_info(PARAM_VOLUME,                "volume",              "Patch Vol",   PARAM_TYPE_REAL, -1, 127, 100,   0, 0,  8, 0, update_volume,              NULL,               NULL);
	/*              index                         name                   label          type             cc  lim  ccv ofst ix leap    callback                    labels              names */
	init_param_info(PARAM_FILTER_CUTOFF,         "filter_cutoff",       "Cutoff",      PARAM_TYPE_REAL,  1, 127,  64,   0, 0, 12, 0, update_filter_cutoff,       NULL,               NULL);
	init_param_info(PARAM_FILTER_RESONANCE,      "filter_resonance",    "Resonance",   PARAM_TYPE_INT,  -1, 127,  51,   0, 0,  8, 0, update_filter_resonance,    NULL,               NULL);
	init_param_info(PARAM_FILTER_SMOOTHING,      "filter_smoothing",    "Smoothing",   PARAM_TYPE_INT,  -1, 127,  16,   0, 0,  8, 0, update_filter_smoothing,    NULL,               NULL);
	init_param_info(PARAM_FILTER_KEYFOLLOW,      "filter_keyfollow",    "KeyFollow",   PARAM_TYPE_DTNT, -1,   4,   4,   0, 0,  1, 0, update_filter_keyfollow,    keyfollow_labels,   keyfollow_names);
	...
	init_param_info(PARAM_FILTER_GAIN,           "filter_gain",         "Filter Gain", PARAM_TYPE_REAL, -1, 127, 108,   0, 0,  8, 0, update_filter_gain,         NULL,               NULL);
	init_param_info(PARAM_FILTER_ENV_AMOUNT,     "filter_env_amount",   "Env Amt",     PARAM_TYPE_REAL, -1, 127,  24,   0, 0, 12, 0, update_filter_env_amount,   NULL,               NULL);
	...
	init_param_info(PARAM_FILTER_ATTACK,         "filter_attack",       "Attack",      PARAM_TYPE_INT,  -1, 127,   8,   0, 0,  8, 0, update_filter_attack,       NULL,               NULL);
	init_param_info(PARAM_FILTER_DECAY,          "filter_decay",        "Decay",       PARAM_TYPE_INT,  -1, 127,  57,   0, 0,  8, 0, update_filter_decay,        NULL,               NULL);
	...
	init_param_info(PARAM_FILTER_RELEASE,        "filter_release",      "Release",     PARAM_TYPE_INT,  -1, 127, 111,   0, 0,  8, 0, update_filter_release,      NULL,               NULL);
	/*              index                         name                   label          type             cc  lim  ccv ofst ix leap    callback                    labels              names */
	...
	init_param_info(PARAM_CHORUS_TIME,           "chorus_time",         "Delay Time",  PARAM_TYPE_INT,  -1, 127, 114,   0, 0, 12, 1, update_chorus_time,         NULL,               NULL);
	...
	init_param_info(PARAM_OSC1_TRANSPOSE,        "osc1_transpose",      "Transpose",   PARAM_TYPE_INT,  -1, 127,  52, -64, 0, 12, 0, update_osc_transpose,       NULL,               NULL);
	init_param_info(PARAM_OSC1_FINE_TUNE,        "osc1_fine_tune",      "Fine Tune",   PARAM_TYPE_INT,  -1, 127, 111, -64, 0,  8, 0, update_osc_fine_tune,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC1_FREQ_LFO,         "osc1_fm_mod",         "FM Mod",      PARAM_TYPE_DTNT, -1,  13,   7,   0, 0,  1, 0, update_osc_freq_lfo,        fm_mod_labels,      fm_mod_names);
	init_param_info(PARAM_OSC1_FREQ_LFO_AMOUNT,  "osc1_fm_mod_amount",  "FM Amt",      PARAM_TYPE_REAL, -1, 127,  76, -64, 0, 12, 0, update_osc_freq_lfo_amount, NULL,               NULL);
	...
	init_param_info(PARAM_OSC1_PHASE_LFO,        "osc1_pm_mod",         "PM Mod",      PARAM_TYPE_DTNT, -1,   9,   4,   0, 0,  1, 0, update_osc_phase_lfo,       mod_labels,         mod_names);
	init_param_info(PARAM_OSC1_PHASE_LFO_AMOUNT, "osc1_pm_mod_amount",  "PM Amt",      PARAM_TYPE_REAL, -1, 127,  34, -64, 0, 15, 0, update_osc_phase_lfo_amount,NULL,               NULL);
	init_param_info(PARAM_OSC1_WAVE_LFO,         "osc1_wave_lfo",       "Wave LFO",    PARAM_TYPE_BBOX, -1,   4,   0,   0, 0,  1, 0, update_osc_wave_lfo,        lfo_labels,         lfo_names);
	...
	init_param_info(PARAM_OSC2_WAVE,             "osc2_wave",           "Wave",        PARAM_TYPE_DTNT, -1,  27,  13,   0, 1,  1, 0, update_osc_wave,            wave_labels,        wave_names);
	...
	init_param_info(PARAM_OSC2_TRANSPOSE,        "osc2_transpose",      "Transpose",   PARAM_TYPE_INT,  -1, 127,  52, -64, 1, 12, 0, update_osc_transpose,       NULL,               NULL);
	init_param_info(PARAM_OSC2_FINE_TUNE,        "osc2_fine_tune",      "Fine Tune",   PARAM_TYPE_INT,  -1, 127, 111, -64, 1,  8, 0, update_osc_fine_tune,       NULL,               NULL);
	init_param_info(PARAM_OSC2_PITCHBEND,        "osc2_pitchbend",      "Pitchbend",   PARAM_TYPE_REAL, -1, 127,  76, -64, 1, 12, 0, update_osc_pitchbend,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC2_FREQ_LFO,         "osc2_fm_mod",         "FM Mod",      PARAM_TYPE_DTNT, -1,  13,   8,   0, 1,  1, 0, update_osc_freq_lfo,        fm_mod_labels,      fm_mod_names);
	init_param_info(PARAM_OSC2_FREQ_LFO_AMOUNT,  "osc2_fm_mod_amount",  "FM Amt",      PARAM_TYPE_REAL, -1, 127,  76, -64, 1, 12, 0, update_osc_freq_lfo_amount, NULL,               NULL);
	...
	init_param_info(PARAM_OSC2_PHASE_LFO,        "osc2_pm_mod",         "PM Mod",      PARAM_TYPE_DTNT, -1,   9,   3,   0, 1,  1, 0, update_osc_phase_lfo,       mod_labels,         mod_names);
	init_param_info(PARAM_OSC2_PHASE_LFO_AMOUNT, "osc2_pm_mod_amount",  "PM Amt",      PARAM_TYPE_REAL, -1, 127,  94, -64, 1, 15, 0, update_osc_phase_lfo_amount,NULL,               NULL);
	init_param_info(PARAM_OSC2_WAVE_LFO,         "osc2_wave_lfo",       "Wave LFO",    PARAM_TYPE_BBOX, -1,   4,   0,   0, 1,  1, 0, update_osc_wave_lfo,        lfo_labels,         lfo_names);
	...
	init_param_info(PARAM_OSC3_WAVE,             "osc3_wave",           "Wave",        PARAM_TYPE_DTNT, -1,  27,  15,   0, 2,  1, 0, update_osc_wave,            wave_labels,        wave_names);
	...
	init_param_info(PARAM_OSC3_INIT_PHASE,       "osc3_init_phase",     "Init Phase",  PARAM_TYPE_REAL, -1, 127,  33,   0, 2, 16, 0, update_osc_init_phase,      NULL,               NULL);
	init_param_info(PARAM_OSC3_TRANSPOSE,        "osc3_transpose",      "Transpose",   PARAM_TYPE_INT,  -1, 127,  52, -64, 2, 12, 0, update_osc_transpose,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC3_PITCHBEND,        "osc3_pitchbend",      "Pitchbend",   PARAM_TYPE_REAL, -1, 127,  76, -64, 2, 12, 0, update_osc_pitchbend,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC3_PHASE_LFO,        "osc3_pm_mod",         "PM Mod",      PARAM_TYPE_DTNT, -1,   9,   4,   0, 2,  1, 0, update_osc_phase_lfo,       mod_labels,         mod_names);
	init_param_info(PARAM_OSC3_PHASE_LFO_AMOUNT, "osc3_pm_mod_amount",  "PM Amt",      PARAM_TYPE_REAL, -1, 127,  34, -64, 2, 15, 0, update_osc_phase_lfo_amount,NULL,               NULL);
	init_param_info(PARAM_OSC3_WAVE_LFO,         "osc3_wave_lfo",       "Wave LFO",    PARAM_TYPE_BBOX, -1,   4,   0,   0, 2,  1, 0, update_osc_wave_lfo,        lfo_labels,         lfo_names);
	...
	init_param_info(PARAM_OSC4_WAVE,             "osc4_wave",           "Wave",        PARAM_TYPE_DTNT, -1,  27,  24,   0, 3,  1, 0, update_osc_wave,            wave_labels,        wave_names);
	...
	init_param_info(PARAM_OSC4_INIT_PHASE,       "osc4_init_phase",     "Init Phase",  PARAM_TYPE_REAL, -1, 127,  11,   0, 3, 16, 0, update_osc_init_phase,      NULL,               NULL);
	init_param_info(PARAM_OSC4_TRANSPOSE,        "osc4_transpose",      "Transpose",   PARAM_TYPE_INT,  -1, 127,  40, -64, 3, 12, 0, update_osc_transpose,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC4_PITCHBEND,        "osc4_pitchbend",      "Pitchbend",   PARAM_TYPE_REAL, -1, 127,  76, -64, 3, 12, 0, update_osc_pitchbend,       NULL,               NULL);
	...
	init_param_info(PARAM_OSC4_PHASE_LFO,        "osc4_pm_mod",         "PM Mod",      PARAM_TYPE_DTNT, -1,   9,   3,   0, 3,  1, 0, update_osc_phase_lfo,       mod_labels,         mod_names);
	init_param_info(PARAM_OSC4_PHASE_LFO_AMOUNT, "osc4_pm_mod_amount",  "PM Amt",      PARAM_TYPE_REAL, -1, 127,  94, -64, 3, 15, 0, update_osc_phase_lfo_amount,NULL,               NULL);
